...
- `get_podcast_transcript(item, config)`: Extracts the MP3 audio URL from `item["attributes"]` (JSON field with an `enclosures` array, e.g. `{"enclosures":[{"url":"...","type":"audio/mpeg",...}]}`); falls back to `item["link"]` only if no enclosure URL is found. Passes the audio URL directly to AssemblyAI for transcription, writes plain text to `{data.directory}/podcasts/{id}.txt`; skips if file already exists; returns path or `None`
