...
- `fetchtranscript.api_key`: API key for YouTube transcript fetching via fetchtranscript.com; may be either a single string or a list of API keys tried in order when quota is exhausted
...
- **YouTube news**: Fetched from DB → `fetchtranscript.com` API transcript fetch (GET `/v1/transcripts/{video_id}?format=text`) using `fetchtranscript.api_key` as either one key or an ordered list of keys; on quota-exhausted responses, the code retries with the next key → raw transcript written to `{data.directory}/youtube/{id}.txt` → LLM synthesis → written to `{data.directory}/news/{id}.md` → synthesis converted to HTML and stored in `entry.content` in SQLite
...
- **Global synthesis**: Check if synthesis file exists at `{data.directory}/synthesis/YYYY-MM-DD.md` (daily) or `{data.directory}/synthesis/YYYY-Week_XX.md` (weekly) → if exists, read existing file; if not exists, merge all `{data.directory}/news/{id}.md` files for the period and send to LLM → structured Markdown digest focused on key period takeaways and recurring stories covered by multiple sources, not a feed-by-feed inventory → written to file → (unless `--no-send` flag is set) markdown content converted to bilingual HTML (English + French) via small LLM model → sent via email to configured recipient
...
- `classify_news(category_name, config, link=None)`: Routes a news item to `"youtube"`, `"podcast"`, or `"regular"`. First checks category name against `feed_categories`; if the category resolves to `"regular"` but the item's `link` is a YouTube URL (detected via `is_youtube_url`), returns `"youtube"` so the item is processed as a YouTube video (Shorts are still skipped inside `process_youtube_news`).
- `is_youtube_url(url)`: Returns `True` if the URL hostname is a YouTube domain (`youtube.com`, `www.youtube.com`, `m.youtube.com`, `youtu.be`). Used by `classify_news` to detect YouTube links in non-YouTube feed categories.
...
- `get_fetchtranscript_api_keys(config)`: Normalizes `fetchtranscript.api_key` into a list of non-empty API keys so config may use either a single string or a list
- `is_fetchtranscript_quota_error(response)`: Detects FetchTranscript quota-exhausted responses so YouTube transcript fetching can fall back to the next configured API key
- `get_youtube_transcript(item, config)`: Downloads transcript via `fetchtranscript.com` API, uses GET request to `/v1/transcripts/{video_id}?format=text`, accepts `fetchtranscript.api_key` as either a string or list of strings, retries with the next key when a quota-exhausted response is returned, writes plain text to `{data.directory}/youtube/{id}.txt`; skips if file already exists; returns path or `None`
- `process_youtube_news(item, config)`: Entry point for YouTube items; silently skips YouTube Shorts (URLs containing `/shorts/`) with no transcript or synthesis. For regular videos: skips if synthesis (`{data.directory}/news/{id}.md`) already exists; otherwise calls `get_youtube_transcript` (which reuses cached `.txt` if present), synthesizes the transcript via LLM, writes to `{data.directory}/news/{id}.md`, and calls `update_db_content`. The transcript and synthesis are tracked independently — a cached transcript does **not** suppress synthesis if the `.md` file is missing. Logs "cached" vs "fetched" at INFO level so the transcript source is always visible. Also guards against empty transcript files before calling the LLM.
...
- `make_global_synthesis(syntheses, frequency, config)`: Sends all collected syntheses to the LLM with instructions to classify each item as either **TYPE A — Current News** (recent events, announcements, launches) or **TYPE B — Tech Articles** (evergreen deep dives, tutorials, opinion pieces). Produces a structured Markdown digest with three top-level sections: `## Highlights` (up to 3–5 cross-source takeaways prioritizing important stories mentioned by multiple feeds), `## News` (selected high-signal TYPE A developments grouped under dynamic topic subsections — e.g. AI, Cloud, Cybersecurity — with same-story coverage from multiple sources merged into one entry and source convergence called out), and `## Tech Articles & Deep Dives` (selected high-signal TYPE B items as single bullet points with Markdown link, **feed name in bold**, and one-sentence description). Low-signal TYPE A and TYPE B items may be omitted so the digest surfaces what not to miss rather than listing every item. Returns the synthesis string or `None`.
...
- **FreshRSS bilingual HTML compatibility**: Do NOT use JavaScript or hidden `<div style='display:none'>` for language toggling in per-item HTML content (`convert_news_synthesis_to_html`). FreshRSS's article viewer does not execute JavaScript, causing the French content to remain permanently hidden. Instead, render both English and French versions sequentially in the same HTML, separated by an `<hr>` tag. Both versions must always be fully visible.
- **FetchTranscript API key rotation**: `fetchtranscript.api_key` can be either a single string or a list of strings. `get_youtube_transcript` normalizes it to a list and retries the next key when FetchTranscript returns HTTP 402 (insufficient quota).
