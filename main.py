...
def make_global_synthesis(syntheses, frequency, config):
    ...
    prompt = (
        ...
        f"1. A short introductory paragraph setting the context for the period.\n\n"
        f"2. A main section '## Current News' containing all TYPE A items, grouped by theme or category. "
        f"For each group use a ### subsection and write a comprehensive synthesis that captures:\n"
        f"   - All main points and key facts\n"
        f"   - Important context and background information\n"
        f"   - Specific data, statistics, and numbers\n"
        f"   - Key quotes and statements\n"
        f"   - Implications, consequences, and technical details\n"
        f"   - Relevant dates, locations, and entities\n\n"
        f"3. A final section '## Tech Articles & Deep Dives' containing all TYPE B items. "
        f"For each item write a single concise bullet point (one or two sentences) with the article title "
        f"(as a Markdown link using the URL provided) and a brief description of what it covers. "
