...
def make_global_synthesis(syntheses, frequency, config):
    ...
    prompt = (
        ...
        f"{items_block}"
        f"Your task:\n"
        f"First, classify every item into one of two types:\n"
        f"  - TYPE A — CURRENT NEWS: items that report a recent event, announcement, or development "
        f"(e.g. a company launching a product, a model release, a funding round, a policy change, a security incident).\n"
        f"  - TYPE B — TECH ARTICLES: items that are evergreen or timeless in nature — deep dives, tutorials, "
        f"technology presentations, opinion pieces, how-it-works explanations — content that is not tied to a specific recent event.\n\n"
        f"Then produce a structured Markdown document organised as follows:\n\n"
        f"1. A single-line context sentence for the period (no long intro paragraph).\n\n"
        f"2. A main section '## Current News' containing all TYPE A items, grouped by theme or category. "
        f"For each group use a ### subsection. "
        f"Present each item as a tight set of bullet points — NO prose paragraphs. Bullet points must cover:\n"
        f"   - What happened (the core fact)\n"
        f"   - Who is involved\n"
        f"   - Key data, numbers, or statistics\n"
        f"   - Notable quotes (if any)\n"
        f"   - Implications or consequences\n"
        f"   Prioritise information density: every bullet must carry a distinct piece of information. "
        f"Omit filler sentences. Include the article title as a Markdown link.\n\n"
        f"3. A final section '## Tech Articles & Deep Dives' containing all TYPE B items. "
        f"For each item write a single bullet point with: the article title as a Markdown link, "
        f"the feed name in bold (e.g. **Feed Name**), and one sentence describing what it covers. "
        f"Do not write long syntheses for these items — the goal is simply to remind the reader they exist.\n\n"
        f"Additional rules:\n"
        f"- Ensure no TYPE A item is lost or omitted. Completeness and depth matter for current news.\n"
        f"- If you are uncertain whether an item is TYPE A or TYPE B, default to TYPE A.\n"
        f"- Use bullet points everywhere. Avoid prose paragraphs except for the single-line context sentence.\n"
        f"- Regardless of the original language of any article, your entire response must be in English.\n"
    )
